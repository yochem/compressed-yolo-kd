...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    LOGGER.info(f"self-compressing hyperparameter: {hyp.get('comp')}")
    ...
    print([x.__class__.__name__ for x in model.model.children()])
    exit()
    ...
    if opt.teacher_weight:
        ...
        stu_feature_adapt = nn.Sequential(
            nn.Conv2d(
                ...
                3,
                padding=1,
                stride=int(student_out_size / teacher_out_size),
            ),
            nn.ReLU(),
        ).to(device)
