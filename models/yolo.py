...
class DetectionModel(BaseModel):
    ...
    def __init__(
        ...
        self.model, self.save, self.anchors = parse_model(
            deepcopy(self.yaml), ch=[ch], get_anchor=True
        )  # model, savelist
    ...
    def _get_imitation_mask(self, x, targets, iou_factor=0.5):
        ...
        for i in range(len(targets)):
            gt_boxes[int(targets[i, 0].data)] += [
                targets[i, 2:].clone().detach().unsqueeze(0)
            ]
        ...
        for i in range(batch_size):
            ...
            IOU_map = find_jaccard_overlap(anchors, gt_boxes[i], 0).view(
                out_size, out_size, self.num_anchors, num_obj
            )
            max_iou, _ = IOU_map.view(-1, num_obj).max(dim=0)
            mask_img = torch.zeros(
                [out_size, out_size], dtype=torch.int64, requires_grad=False
            ).type_as(x)
...
def parse_model(d, ch, get_anchor=False):  # model_dict, input_channels(3)
    ...
    anchors = sum(anchors, [])
    anchors = [
        (anchors[i] // 8, anchors[i + 1] // 8) for i in range(0, len(anchors), 2)
    ]
    if get_anchor:
